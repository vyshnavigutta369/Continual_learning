...
class WeightedSampler(Sampler):
    ...
    def get_distribution(self):
        ...
        if self.min_val:
            weights = 1 / (self.weights[:self.num_samples] + 1e-7)
        else:
            weights = self.weights[:self.num_samples]
        
        # # print (collections.Counter(weights)[0])
        # weights*= self.class_weights[:self.num_samples]
        # # print (collections.Counter(weights)[0])
        # distribution = F.log_softmax(
        #     self.beta * weights, dim=0).data.exp()
        # # print ('3: ', F.softmax(self.beta * weights))
        ...
        distribution= self.class_weights[:self.num_samples]
        # print ('3: ', collections.Counter(weights.cpu().numpy())[0])
        return distribution
