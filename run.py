...
def create_args():
    ...
    parser.add_argument('--plot_dir', type=str, default="plots_and_tables/",
                         help="Save experiments results in dir for future plotting!")
    ...
    parser.add_argument('--reuse_replay_ixs', type=int, default=0, metavar='N', help='reuse_replay_ixs')
    ...
    parser.add_argument('--buffer_update', type=str, default='partial_every', help="buffer-update")
...
if __name__ == '__main__':
    ...
    metric_keys = ['acc','mem','time','epochs', 'avg_acc_scores', 'fwt_scores', 'bwt_scores', 'gamma_t', 'replay_item_count']
    save_keys = ['global', 'pt', 'pt-local']
    global_only = ['mem','time','epochs', 'avg_acc_scores', 'fwt_scores', 'bwt_scores', 'gamma_t', 'replay_item_count']
    avg_metrics = {}
    ...
    for r in range(start_r, args.repeat):
        ...
        # evaluate model
        avg_metrics, r_matrix = trainer.evaluate(avg_metrics)    
        #avg_metrics['avg_acc_scores'], avg_metrics['fwt_scores'], avg_metrics['bwt_scores'], avg_metrics['gamma_t'] = trainer.compute_metrics(r_matrix, oracle_acc)

        # print (avg_acc_scores, fwt_scores, bwt_scores, gamma_t)

        # np.save(args.log_dir+'/results-acc/r_matrix.npy', r_matrix)
        # save results
