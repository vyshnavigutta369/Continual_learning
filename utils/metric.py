...
def per_class_plots(per_class_accuracy, per_class_dist_shift, labels_to_names, class_mapping, epochs_of_interest, replay_size, avg_acc=None, task_acc=None, base_path = 'plots_and_tables/', is_oracle=False):
    ...
    if avg_acc is not None:    
        ...
        plot_multiline(to_plot, labels, plot_colors, xticks = epochs_of_interest, xlabel='epochs', ylabel='average class accuracy', y_lim_bottom=0, title=title, fig_name = base_path+'avg_accuracy.png')
...
class MMD(object):
    ...
    def polynomial_kernel(self, X, Y, degree=2, gamma=1, coef0=0):
        ...
        if len(X.shape)==3:
            return ((gamma*(torch.einsum("abc, edc -> aebd", X, Y)) + coef0) ** degree)
    ...
    def mmd_poly(self, X, Y, degree=2, gamma=1, coef0=0):
        ...
        if len(X.shape)==3:
            return XX.mean(dim=(2,3)) + YY.mean(dim=(2,3)) - 2 * XY.mean(dim=(2,3))
