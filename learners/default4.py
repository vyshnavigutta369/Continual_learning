...
class NormalNN(nn.Module):
            ...
            if self.replay:
                ...
                if self.is_custom_replay_loader:
                    ...
                    if self.epoch==self.epochs_of_interest[-1] and self.is_batch_sampler:
                        self.replay_loader.batch_sampler.extend(len(self.train_dataset)) ## Full replay addition, TODO if u want to limit amount of replay being added.
                        # print ('length of self.replay_loader0: ', self.replay_loader.batch_sampler.num_samples)
                        
                    # print ('length of self.replay_loader2: ', len(self.replay_loader.dataset))
    ...
    def split_by_class(self, indices, labels, old_features=None, new_features=None, predictions= None, new_datas_train=None, old_features_train=None, new_features_train=None):
        ...
        if predictions is not None:
            ...
            # print ('1:', self.per_class_correct_predictions.keys())
            # print ('2:', cl_split.keys())
            if self.per_class_correct_predictions:
                self.per_class_correct_predictions = { k: {**self.per_class_correct_predictions.get(k,{}), **cl_split.get(k,{})} for k in list(cl_split.keys())+list(self.per_class_correct_predictions.keys())}
            else:
                self.per_class_correct_predictions = cl_split
            
            # print ('3:', self.per_class_correct_predictions.keys())
    ...
    def scale_distribution(self, data, classes, rnge=(1,10)):
        ...
        self.class_replay_ratios =  { cl: math.floor(scale_to(x,r_min, r_max, t_min, t_max)) for cl,x in zip(classes, data) if cl in self.new_classes}
    ...
    def analyze(self):
        ...
        if (not self.replay and self.epoch==self.epochs_of_interest[-1]) or (self.replay and self.epoch in self.epochs_of_interest):
            ...
            self.class_accuracy_epoch, self.class_dist_shift_epoch, self.per_class_accuracy, self.per_class_dist_shift = distribution_shift_comparison(self.labels_to_names, self.class_mapping, self.per_class_new_features, self.per_class_old_features, self.per_class_correct_predictions, self.per_class_accuracy, self.per_class_dist_shift, self.plot_dir)
            ...
            if not self.is_oracle: 
                ...
                elif int(self.class_weighting_with)==13: ## sim shift
                    if self.replay:
                ...
                # print ('class_replay_weights before: ', { self.labels_to_names[self.class_mapping[cl]]: self.class_replay_weights[cl] for cl in self.class_replay_weights})
                # counter = dict(Counter(self.class_replay_weights.values()))
                # self.class_replay_weights = { i: self.class_replay_ratios[i]/counter[self.class_replay_ratios[i]] for i in self.class_replay_ratios}
    ...
    def learn_batch(self, train_loader, train_dataset, replay_dataset, model_save_dir, val_target, task, val_loader=None):
        ...
        if self.replay:
            plots(self.per_class_accuracy, self.per_class_dist_shift, self.labels_to_names, self.class_mapping, self.epochs_of_interest, self.steps_of_interest, self.times_of_interest, self.replay_size,  self.avg_acc,  base_path=self.plot_dir+'_after/', is_oracle=True)
