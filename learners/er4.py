...
class TR(NormalNN):
    ...
    def initialise_replay_loader(self, weights=None):
        ...
        if not self.replay_strategy or weights is None:
            sampler = dataloaders.WeightedSampler(num_samples=len(self.replay_dataset), replay_strategy=self.replay_strategy)
        else:
            sampler = dataloaders.WeightedSampler(num_samples=len(self.replay_dataset), weights_init=weights, replay_strategy=self.replay_strategy)
        ...
        print ('length of self.replay_loader3: ', len(self.replay_loader.dataset))
    ...
    def batch_data_weighting(self, output, labels, indices, train_data_batch_size):
        ...
        if self.epoch==self.epochs_of_interest[-1]:
            ...
            if self.is_weighted_sampler:
                if hasattr(self, 'replay_loader') and self.is_batch_sampler:
                ...
                # elif hasattr(self, 'replay_loader'):
                #     self.replay_loader.sampler.update_weights(batch_new_weights=batch_weights, indices=len(self.replay_loader.batch_sampler)+indices[:train_data_batch_size])
                
            elif not self.is_weighted_sampler or not hasattr(self, 'replay_loader'):
        ...
        if self.replay:
            ...
            if self.is_weighted_sampler:
                ...
                # elif self.epoch==self.epochs_of_interest[-1]:
    ...
    def learn_batch(self, train_loader, train_dataset, replay_dataset, model_save_dir, val_target, task, val_loader=None):
        ...
        if need_train:
            ...
            for epoch in range(0,self.config['schedule'][-1]+1):
                ...
                if epoch>0:
                    for i, data  in enumerate(self.train_loader):
                        ...
                        if not self.is_weighted_sampler:
            ...
            if self.replay:
                plots(self.per_class_accuracy, self.per_class_dist_shift, self.labels_to_names, self.class_mapping, self.epochs_of_interest, self.steps_of_interest, self.times_of_interest, self.replay_size,  self.avg_acc, base_path=self.plot_dir+'_after/')
    ...
    def extend_replay(self):
        ...
        if not self.is_weighted_sampler:
            ...
            self.replay_dataset.extend(self.train_dataset, class_replay_weights= self.class_replay_weights)
            if not hasattr(self, 'replay_loader'):
                try:
                    ...
                    self.initialise_replay_loader()
            if self.is_batch_sampler:
                self.replay_loader.batch_sampler.sampler.num_samples=len(self.replay_dataset) 
                self.replay_loader.batch_sampler.sampler.class_weights= torch.Tensor(self.replay_dataset.weights).cuda()
            # else:
            #     self.replay_loader.sampler.num_samples=len(self.replay_dataset) 
            #     self.replay_loader.sampler.class_weights= torch.Tensor(self.replay_dataset.weights).cuda()
