...
class TR(NormalNN):
    ...
    def batch_data_weighting(self, output, labels, indices, train_data_batch_size):
        ...
        if self.replay:
            ...
            elif self.is_custom_replay_loader and self.epoch==self.epochs_of_interest[-1]:
                self.replay_loader.sampler.update_weights(batch_new_weights=replay_batch_weights, indices=indices[train_data_batch_size:])
                
            elif self.replay_strategy and self.epoch==self.epochs_of_interest[-1]:
                        ...
                        if self.is_dual_data_loader:
                            ...
                            if self.replay:
                                try:
                                    ...
                                    # print ('hellooo')
                                    # self.replay_loader_iter = iter(self.replay_loader)
                                    # x_r, y_r, replay_indices =  next(self.replay_loader_iter)
                                    pass
    ...
    def extend_replay(self):
        ...
        if self.is_dual_data_loader and not self.replay_strategy:
            ...
            if self.replay_strategy:
                ...
                self.replay_dataset.extend(self.train_dataset, class_replay_weights= self.class_replay_weights, replay_strategy=self.replay_strategy)

            self.num_replay_samples = self.num_replay_samples_init if self.num_replay_samples_init!=-1 else len(self.replay_dataset)
            print ('num_replay_samples:', self.num_replay_samples)
            if self.is_custom_replay_loader:
                if not hasattr(self, 'replay_loader'):
                    print ('data_weights', self.data_weights[self.data_indices])
                ...
                if self.is_batch_sampler:
                    ...
                    # print ('1:', self.replay_loader.sampler.class_weights)
                     
                # s = {i: set() for i in range(10)}
                # sampler = dataloaders.WeightedSampler(num_samples=self.num_replay_samples, replay_strategy=self.replay_strategy)
                # self.replay_loader2 = DataLoader(dataset=self.replay_dataset, sampler=self.sampler, batch_size=self.num_replay_samples, num_workers=self.workers)
                # for x,y,inds in self.replay_loader:
                #     for l,ind in zip(y,inds):
                #         s[int(l)].add(int(ind))

                # print ('3:', self.replay_dataset.class_weights)
                # print ('2:', self.replay_loader.sampler.weights)
                
                # print ('check_replay_counts after replay loader init: ', {self.labels_to_names[self.class_mapping[cl]]: len(s[cl]) for cl in s})

            # if self.class_replay_weights is not None or self.replay_strategy:
